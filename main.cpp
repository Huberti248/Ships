...
#define TURN_TIMEOUT_IN_MS 10000
...
Uint32 timeoutCallback(Uint32 interval, void* param)
{
    ...
    for (int i = 0; i < clients.size(); ++i) {
        if (i != index && clients[index].gameId == clients[i].gameId) {
            clients[i].playerTurn = !clients[i].playerTurn;
            clients[i].timeoutId = SDL_AddTimer(TURN_TIMEOUT_IN_MS, timeoutCallback, (int*)i);
        }
    }
    clientsMutex.unlock();
    return 0;
                    ...
                    if (received > 0) {
                        if (msg.size() >= 12 && msg.substr(0, 12) == "changeState ") {
                            ...
                            if (clients[i].playerTurn) {
                                ...
                                if (redCount == 18) {
                                    SDL_RemoveTimer(clients[i].timeoutId);
                                    ...
                                    if (clients[i].opponentBoard[index].c == SDL_Color({ 255, 0, 0 })) {
                                        ...
                                        for (int j = 0; j < clients.size(); ++j) {
                                            if (j != i && clients[j].gameId == clients[i].gameId) {
                                                ...
                                                clients[j].timeoutId = SDL_AddTimer(TURN_TIMEOUT_IN_MS, timeoutCallback, (int*)j);
                                            }
                                        }
                                    }
                                }
...
{
    setState(State::YourTurnGameplay);
    return 0;
}
...
gameBegin:
    ...
    while (running) {
        if (state == State::ShipPlacement) {
            ...
            if (!wfp.first) {
                ...
                if (answer == "Gameplay") {
                    ...
                    if (answer == "your") {
                        ...
                        setState(State::NotYoursTurnGameplay);
                    }
                }
            }
            wfp.first = false;
            ...
            std::string answer;
            receive(socket, answer);
            if (answer.size() >= 13 && answer.substr(0, 13) == "notYoursTurn ") {
                setState(State::NotYoursTurnGameplay);
                nytg.endTurn = false;
            }
            SDL_SetRenderDrawColor(renderer, BG_COLOR);
